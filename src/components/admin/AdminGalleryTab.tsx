...
export const AdminGalleryTab: React.FC<AdminGalleryTabProps> = ({
    currentUser, dbStats, onAddGallery, onUpdateArchivePhone,
}) => {
    ...
    const handleGallerySubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!galleryFile) { toast('Please select a photo or video to upload.', 'error'); return; }
    ...
    const handleBulkGalleryUpload = async () => {
        ...
        if (files.length === 0) { toast('No valid image or video files selected.', 'error'); return; }
        ...
        <section className="space-y-6 animate-in fade-in">
            ...
            <div className="p-6 bg-stone-50 rounded-[2rem] border border-stone-200 mb-8">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    <input id="admin-archive-phone" placeholder="e.g. +15551234567" className="flex-1 p-4 border border-stone-200 rounded-2xl text-base bg-white outline-none focus:ring-2 focus:ring-[#2D4635]/20" value={archivePhoneLocal} onChange={e => setArchivePhoneLocal(e.target.value)} />
                    <button type="button" onClick={async () => { if (isSavingArchivePhone) return; setIsSavingArchivePhone(true); try { await Promise.resolve(onUpdateArchivePhone(archivePhoneLocal)); toast('Gallery config updated', 'success'); } finally { setIsSavingArchivePhone(false); } }} disabled={isSavingArchivePhone} aria-busy={isSavingArchivePhone} className="px-6 py-4 bg-[#2D4635] text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg disabled:opacity-70 disabled:cursor-not-allowed whitespace-nowrap">
                ...
                <div className="p-6 bg-[#2D4635]/5 rounded-3xl border border-[#2D4635]/10">
                    <div className="relative group">
                        <label htmlFor="admin-bulk-gallery-files" className="block cursor-pointer">
                            <input id="admin-bulk-gallery-files" type="file" accept="image/*,video/*" multiple onChange={e => setBulkFiles(e.target.files)} className="absolute inset-0 opacity-0 cursor-pointer z-10 w-full h-full" aria-label="Select multiple photos or videos for bulk upload" />
                            <div className="w-full h-24 border-2 border-dashed border-[#2D4635]/30 rounded-[2rem] flex flex-col items-center justify-center gap-2 text-[#2D4635]/60 group-hover:border-[#2D4635] group-hover:text-[#2D4635] transition-all bg-white/50">
                                ...
                                <span className="text-[9px] font-black uppercase tracking-widest">
                                    {bulkFiles && bulkFiles.length > 0 ? `${bulkFiles.length} files selected` : 'Drag & Drop or Click to Select Multiple'}
                                </span>
                            </div>
                        </label>
                    </div>
                        ...
                        <div className="mt-4 space-y-3">
                            <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
                                {(Array.from(bulkFiles) as File[]).slice(0, 10).map((file, idx) => (
                                    <span key={idx} className="px-3 py-1 bg-white rounded-full text-[9px] font-medium text-stone-600 truncate max-w-[150px]">{file.name}</span>
                                ))}
                                {bulkFiles.length > 10 && (
                                    <span className="px-3 py-1 bg-white/50 rounded-full text-[9px] font-medium text-stone-500">+{bulkFiles.length - 10} more</span>
                                )}
                            </div>
                            <label htmlFor="admin-bulk-gallery-caption" className="sr-only">Caption for all bulk upload files (optional)</label>
                            <input id="admin-bulk-gallery-caption" placeholder="Caption for all (optional)..." aria-label="Caption for all bulk upload files (optional)" className="w-full p-3 border border-stone-200 rounded-xl text-base outline-none bg-white" value={galleryForm.caption} onChange={e => setGalleryForm({ ...galleryForm, caption: e.target.value })} />
                            {uploadProgress.total > 0 && (
                                <div className="space-y-2" role="status" aria-live="polite" aria-busy={uploadProgress.current < uploadProgress.total}>
                                    ...
                                    <div className="w-full h-2 bg-white rounded-full overflow-hidden">
                                        <div className="h-full bg-[#2D4635] transition-all duration-300" style={{ width: `${(uploadProgress.current / uploadProgress.total) * 100}%` }} />
                                    </div>
                                    {uploadProgress.errors.length > 0 && (
                                        <p className="text-[9px] text-red-600 font-medium">{uploadProgress.errors.length} file(s) failed to upload</p>
                                    )}
                                </div>
                            )}
                            <button onClick={handleBulkGalleryUpload} disabled={isSubmitting || uploadProgress.total > 0} className="w-full py-3 bg-[#2D4635] text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">
                                {isSubmitting && uploadProgress.total > 0 ? `Uploading ${uploadProgress.current}/${uploadProgress.total}...` : `Upload ${bulkFiles.length} Files to Gallery`}
                            </button>
                            {uploadProgress.errors.length > 0 && (
                                <div className="p-3 bg-red-50 border border-red-200 rounded-xl">
                                    <p className="text-[9px] font-bold text-red-600 uppercase tracking-widest mb-2">{uploadProgress.errors.length} Failed:</p>
                                    <p className="text-[9px] text-red-500 max-h-20 overflow-y-auto whitespace-pre-line">{uploadProgress.errors.join('\n')}</p>
                                </div>
                            )}
                        </div>
                    )}
