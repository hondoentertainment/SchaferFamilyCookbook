...
export interface AdminTriviaTabProps {
    trivia: Trivia[];
...
export const AdminTriviaTab: React.FC<AdminTriviaTabProps> = ({
    trivia, currentUser, onAddTrivia, onDeleteTrivia,
}) => {
    ...
    return (
        <section className="space-y-6 animate-in fade-in">
            ...
            <form onSubmit={async (e) => {
                e.preventDefault();
                if (!triviaForm.question?.trim()) { toast('Question is required.', 'error'); return; }
                ...
                {trivia.map(t => (
                    <div key={t.id} className="flex items-center justify-between p-3 bg-white rounded-2xl mb-2 group border border-stone-100 shadow-sm">
                        <div
                            ...
                            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setEditingTrivia(t); setTriviaForm(t); } }}
                        ...
                        <button
                            onClick={async () => {
                                ...
                                if (ok) { await onDeleteTrivia(t.id); toast('Trivia deleted', 'success'); }
                            }}
