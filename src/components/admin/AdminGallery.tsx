...
export const AdminGallery: React.FC<AdminGalleryProps> = ({
    ...
    return (
        <section id="admin-panel-gallery" role="tabpanel" aria-labelledby="admin-tab-gallery" className="bg-white/70 backdrop-blur-xl rounded-[2rem] md:rounded-[3rem] p-6 md:p-12 border border-stone-200/50 shadow-[0_20px_40px_rgba(45,70,53,0.08)] overflow-hidden relative animate-in fade-in slide-in-from-bottom-4">
            <h2 className="text-3xl font-serif italic text-[#2D4635] mb-10">Family Archive</h2>
            ...
            <div className="space-y-6 animate-in fade-in">
                ...
                <div className="p-6 bg-emerald-50/80 backdrop-blur-sm rounded-3xl border border-emerald-100 flex items-start gap-4 mb-4 shadow-sm">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-emerald-700 font-serif italic mt-1 leading-relaxed">
                            ...
                            <span className="font-bold not-italic font-sans">{dbStats.archivePhone || 'Not Configured'}</span>
                        </p>
                    </div>
                </div>

                <div className="p-6 bg-white/50 backdrop-blur-sm rounded-[2rem] border border-stone-200/60 mb-8 shadow-sm">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        <input
                            ...
                            className="flex-1 p-4 border border-stone-200/50 bg-white/80 rounded-2xl text-base outline-none focus:ring-2 focus:ring-[#2D4635]/20 transition-colors"
                        ...
                        <button
                            ...
                            className="px-6 py-4 bg-[#2D4635] text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg disabled:opacity-70 disabled:cursor-not-allowed hover:bg-[#1e2f23] transition-colors whitespace-nowrap"
                        >
                ...
                <form onSubmit={handleGallerySubmit} className="space-y-4">
                    <div className="relative group">
                        <label htmlFor="admin-gallery-file" className="block cursor-pointer">
                            ...
                            <div className="w-full h-32 border-2 border-dashed border-stone-200/60 bg-white/30 rounded-[2rem] flex flex-col items-center justify-center gap-2 text-stone-400 group-hover:border-[#A0522D] group-hover:bg-[#A0522D]/5 group-hover:text-[#A0522D] transition-all">
                    ...
                    <div>
                        ...
                        <input id="admin-gallery-caption" placeholder="Caption (e.g. Summer BBQ 1985)" className="w-full p-4 border border-stone-200/50 bg-white/80 rounded-2xl text-base outline-none focus:ring-2 focus:ring-[#2D4635]/20 transition-colors" value={galleryForm.caption} onChange={e => setGalleryForm({ ...galleryForm, caption: e.target.value })} />
                    </div>
                    <button type="submit" disabled={!galleryFile || isSubmitting} aria-busy={isSubmitting} className="w-full py-4 bg-[#A0522D] text-white rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg disabled:opacity-70 disabled:cursor-not-allowed hover:bg-[#8B4513] transition-colors">
                ...
                <div className="mt-8 pt-8 border-t border-stone-200/50">
                    ...
                    <div className="p-6 bg-[#2D4635]/5 backdrop-blur-sm rounded-3xl border border-[#2D4635]/10 shadow-inner">
                        <div className="relative group">
                            <label htmlFor="admin-bulk-gallery-files" className="block cursor-pointer">
                                ...
                                />
                                <div className="w-full h-24 border-2 border-dashed border-[#2D4635]/30 rounded-[2rem] flex flex-col items-center justify-center gap-2 text-[#2D4635]/60 group-hover:border-[#2D4635] group-hover:bg-[#2D4635]/5 group-hover:text-[#2D4635] transition-all bg-white/50">
                            ...
                            <div className="mt-4 space-y-3">
                                <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
                                    {(Array.from(bulkFiles) as File[]).slice(0, 10).map((file, idx) => (
                                        <span key={idx} className="px-3 py-1 bg-white/80 backdrop-blur-md rounded-full text-[9px] font-medium text-stone-600 truncate max-w-[150px] shadow-sm">
                                            {file.name}
                                        </span>
                                    ))}
                                    {bulkFiles.length > 10 && (
                                        <span className="px-3 py-1 bg-white/40 backdrop-blur-sm rounded-full text-[9px] font-medium text-stone-500 border border-stone-200/50">
                                ...
                                <input
                                    ...
                                    className="w-full p-3 border border-stone-200/50 bg-white/80 rounded-xl text-base outline-none focus:ring-2 focus:ring-[#2D4635]/20 transition-colors"
                                ...
                                {uploadProgress.total > 0 && (
                                    <div className="space-y-2" role="status" aria-live="polite" aria-busy={uploadProgress.current < uploadProgress.total}>
                                        <div className="flex items-center justify-between text-[10px] font-bold text-[#2D4635] uppercase tracking-widest">
                                ...
                                <button
                                    ...
                                    className="w-full py-3 bg-[#2D4635] text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#1e2f23] transition-colors"
                                >
                                ...
                                {uploadProgress.errors.length > 0 && (
                                    <div className="p-3 bg-red-50/80 backdrop-blur-sm border border-red-200 rounded-xl shadow-sm">
