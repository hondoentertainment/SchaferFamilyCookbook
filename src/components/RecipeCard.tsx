...
export const RecipeCard = memo(({ recipe, onClick, avatarUrl }: RecipeCardProps) => {
    const [imageError, setImageError] = useState(false);
    const hasImage = recipe.image && !imageError;

    return (
        <div
            onClick={() => onClick(recipe)}
            className={`group cursor-pointer relative aspect-[3/4] rounded-[2rem] overflow-hidden bg-stone-200 shadow-md hover:shadow-2xl transition-all duration-500 ${imageError ? 'fallback-gradient' : ''}`}
        >
