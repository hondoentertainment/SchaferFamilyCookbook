...
// Optimization: Memoize the component to prevent unnecessary re-renders when parent state changes.
// Only re-renders if the 'recipe' object or 'onClick' reference changes.
export const RecipeCard = React.memo(({ recipe, contributorAvatar, onClick }: RecipeCardProps) => {
    return (
        <div
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent p-6 flex flex-col justify-end">
                <div className="transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                    ...
                    <p className="text-[10px] text-stone-300 uppercase tracking-widest mt-2 opacity-0 group-hover:opacity-100 transition-opacity delay-100 flex items-center gap-1">
                        By <img src={contributorAvatar} className="w-4 h-4 rounded-full inline-block" alt="" /> {recipe.contributor}
                    </p>
                </div>
            </div>
        </div>
