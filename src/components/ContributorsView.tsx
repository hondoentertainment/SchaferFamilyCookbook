...
export const ContributorsView: React.FC<ContributorsViewProps> = ({ recipes, onSelectContributor }) => {
    const stats = useMemo(() => {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
                {stats.map(([name, stat]) => (
                    <div key={name} className="bg-white rounded-[3rem] p-10 border border-stone-100 shadow-sm hover:shadow-2xl transition-all group relative overflow-hidden text-center">
                        <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${name}`} className="w-28 h-28 rounded-full bg-stone-50 border-8 border-white shadow-xl mx-auto mb-8 group-hover:rotate-6 transition-all" alt={name} />
