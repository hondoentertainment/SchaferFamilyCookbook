        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="add-recipe-modal-title">
            ...
            <div className="relative bg-white rounded-[2rem] md:rounded-[3rem] p-6 md:p-12 border border-stone-200 shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleRecipeSubmit} className="space-y-6">
                        ...
                        {previewUrl && (
                            <div className="relative w-full h-48 rounded-[2rem] overflow-hidden mb-4 border border-stone-100 shadow-inner">
                                <img
                                    ...
                                    alt="Preview"
                                    decoding="async"
                                    onError={(e) => { (e.target as HTMLImageElement).src = getDefaultImageForCategory(recipeForm.category); }}
                                />
                            </div>
                        )}
                        {!previewUrl && (
                    ...
                    <div>
                        <span className="label text-stone-400 ml-2 block mb-1">Scan a recipe card</span>
                        <div className="relative group">
                            <label htmlFor="add-recipe-scan-photo" className="block cursor-pointer">
                                ...
                                />
                                <div className={`w-full py-3 bg-[var(--color-brand)]/10 text-[var(--color-brand)] rounded-2xl label border border-[var(--color-brand)]/20 group-hover:bg-[var(--color-brand)]/20 transition-all text-center ${(isScanLoading || isAICooldownActive) ? 'opacity-50' : ''}`}>
