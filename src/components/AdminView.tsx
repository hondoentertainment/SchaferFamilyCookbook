...
export const AdminView: React.FC<AdminViewProps> = (props) => {
    ...
    const handleMagicImport = async () => {
        ...
        try {
            // Use process.env.GEMINI_API_KEY as defined in vite config
            const apiKey = process.env.GEMINI_API_KEY;
            if (!apiKey) {
                throw new Error("Missing Gemini API Key. Please check .env.local");
            }
            ...
            const response = await ai.models.generateContent({
                ...
                contents: [
                    {
                        role: 'user',
                        parts: [{ text: `Recipe text: ${rawText}` }]
                    }
                ],
                config: {
            ...
            setRawText('');
            alert("Magic Import Successful! Please review the fields.");
        } catch (e: any) {
            console.error("AI Analysis failed:", e);
            alert(`AI Analysis failed: ${e.message || "Unknown error"}. Please enter manually.`);
        } finally {
    ...
    const handleGenerateImage = async () => {
        ...
        try {
            ...
            const ai = new GoogleGenAI({ apiKey });
            // NOTE: Standard SDK usage is via helper or direct REST if not typed yet.
            // Using generic 'generateContent' with tool configuration for Imagen is possible in some versions,
            // but currently @google/genai might not have explicit type for imagen-3.0-generate-001 via models.generateImage yet.
            // We use standard generateContent or verify SDK capabilities. 
            // Assuming the SDK supports `models.generateImage` or similar based on previous context.
            // Adjusting to safe usage: if this fails at runtime, we catch it.
            const response: any = await (ai.models as any).generateImage({ // Casting mainly to avoid TS error if types are outdated
                model: 'imagen-3.0-generate-001',
                prompt: `A delicious, professional food photography shot of ${recipeForm.title}, ${recipeForm.category} dish. High resolution, warm lighting, appetizing.`,
                config: {
                    numberOfImages: 1,
                }
            });
            ...
            if (b64) {
                ...
                throw new Error("No image data in response. Check console.");
            }
        } catch (e: any) {
                        ...
                        <form onSubmit={handleRecipeSubmit} className="space-y-6 pt-8 border-t border-stone-50">
                            <div className="space-y-2">
                                ...
                                <div className="text-center pt-2">
                                    <span className="text-[9px] font-bold text-stone-300 uppercase tracking-widest">- OR -</span>
                                </div>
                                <button type="button" onClick={handleGenerateImage} disabled={isGeneratingImage || !recipeForm.title} className="w-full py-3 bg-[#A0522D]/10 text-[#A0522D] rounded-2xl text-[10px] font-black uppercase tracking-widest border border-[#A0522D]/20 hover:bg-[#A0522D] hover:text-white transition-all disabled:opacity-50 mt-2">
                                ...
                                <div key={name} className="flex flex-col items-center gap-3 p-4 bg-stone-50 rounded-3xl border border-stone-100 hover:shadow-lg transition-all cursor-pointer group relative">
                                    ...
                                    <div className="flex gap-2">
                                        <span
                                            ...
                                            className="text-[9px] uppercase tracking-widest text-[#2D4635] hover:font-bold"
                                        >
                                            Avatar
                                        </span>
                                        <span
                                            ...
                                            className={`text-[9px] uppercase tracking-widest ${role === 'admin' ? 'text-orange-500 font-bold' : 'text-stone-400'} hover:font-bold`}
                                        >
                                            {role}
                                        </span>
                                    </div>
